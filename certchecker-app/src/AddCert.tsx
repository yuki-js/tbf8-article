...
import { Card, Toaster as Toast, Intent, Button, ButtonGroup } from "@blueprintjs/core";
  ...
  function signsend(arr: Uint8Array) {
    ...
    transaction.signAndSend(alice, (e: any) => {
      if (e.isError) {
        ...
        pt.show({
          message: "送信を確認できませんでした。送信自体は成功しているかもしれません。",
          intent: Intent.DANGER
        })
  ...
  return (
    <section>
      <Card style={cardStyle} onDrop={read} onDragOver={ce} onDragEnter={ce}>
        <div id="dragarea" >
          ここにファイルをドラッグするとファイルを署名してブロックチェーンに書き込みます。
        </div>
      </Card>
      <ButtonGroup fill={true} style={cardStyle}>
        <Button rightIcon="arrow-right" onClick={() => history.push("/CheckCert")}>確認はこちら</Button>
      </ButtonGroup>
    </section>
  )
}
