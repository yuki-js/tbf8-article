...
import { Card, Spinner, Text, Drawer, Divider, Button, Intent } from "@blueprintjs/core";
...
export default function AddCert() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const hashList = await api.query.certStore.certificateArray.multi(Array.from({ length: len }, (v, k) => k))
  ...
  function download() {
    downloadBlob(data, "certificate_" + data.hash.toString() + ".pdf", "application/pdf")
  }
  return (
    <section>
      ...
      {
        hashes.map((h, i) => (
          <Card interactive={true} onClick={() => fetch(h)} key={i} style={elemStyle}>
            {h.toString()}
          </Card>
        ))
      }
      ...
      <Drawer isOpen={!!data} onClose={() => setData(null)} style={elemStyle}>
        {data && (<div>
          ...
          <Text>に署名されています</Text>
          <Divider />
          <Button onClick={download} intent={Intent.PRIMARY} icon="import">Download</Button>
        </div>)}
      </Drawer>
    </section>
  )
